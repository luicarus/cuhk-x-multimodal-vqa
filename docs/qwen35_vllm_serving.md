...
| `qwen35_4b_test_vllm_v3`（当前） | vLLM DP=2、TP=1，并行加载 | 32 | 16 | **1.3928 req/s** | TTFT P50 12119 ms；P95 17493 ms；P99 26233 ms（682/682） | **717.984 ms / 请求** | **104.26 / 602.59 s** |

当前 DP2-B32 的 682 条请求分成 22 批（21 批各 32 条，末批 10 条），平均批次大小 31.0，平均批次耗时 22.257 s；批次总耗时 489.665 s，输出 token rate 为 2.9142 tokens/s。批次摊销时间是批次时长除以请求数，不是单请求延迟。

当前结果较 TP2-B32 的吞吐低 2.6%，批次阶段耗时高 2.7%，端到端时间多 34.8 s。与上一轮未启用 TTFT/NVML 监控的 DP2-B32 相比，吞吐从 1.5001 降至 1.3928 req/s（低 7.2%），端到端时间多 42.8 s；这是一轮新 Kaggle 运行，无法区分监控开销与运行环境波动。与此前记录的 DP2-B16 相比，吞吐低 10.1%，但并行初始化让模型加载少 78.3 s，端到端时间少 27.8 s。不同轮次不能视为受控的单变量实验。

当前逐请求 TTFT 覆盖率为 682/682（100%），均值 12353 ms，P50 12119 ms、P95 17493 ms、P99 26233 ms，最大 28431 ms。它来自 vLLM 的 `first_token_latency`，从引擎接收请求开始并包含调度排队；不是整条请求的端到端时延。每条 QA 的 TTFT 样本保存在 `run_summary.json` 的 `latency.ttft_ms.samples`。
...
- **启动占比与残差**：模型加载占端到端总耗时，TP2 逐条、TP2-B32、当前 DP2-B32 分别约为 12.27%、14.66%、17.30%。端到端耗时减去加载和已记录推理阶段后，仍有约 7.86 / 7.76 / 8.67 s 未进一步拆分；这是残差，不能归因到单一组件。
...
| **DP2-B32（当前）** | **4332.8 / 4332.8** | **2418.8 / 2268.8** |

当前 DP2-B32 在 100 ms 间隔下采到 4871 次：GPU0/GPU1 sampled peak used 为 **12941.2 / 13091.2 MiB**（12.64 / 12.78 GiB），sampled minimum free 为 **2418.8 / 2268.8 MiB**。结果保存在 `gpu_memory.peaks.device_polling`，采样范围为模型加载后的预测阶段。它是全设备指标、包含其他进程占用，采样间隔之间的瞬时尖峰可能漏掉；worker allocator 的 `peak_allocated_mib` / `peak_reserved_mib` 仍为 0。不同 Kaggle session 的后台占用也会影响结果。
...
1. 在同一 Kaggle 硬件上重复 DP2-B32 运行 2–3 次，估计吞吐、TTFT 与采样峰值的运行间波动，并判断本次吞吐下降是否可复现。
