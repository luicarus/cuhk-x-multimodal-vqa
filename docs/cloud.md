...
Qwen3.5-4B 对照使用独立的 `notebooks/qwen35-4b-test.ipynb` 和 `artifacts/cloud/qwen35_4b_test_v1.zip`；它不改变本 Notebook、7B 权重或 baseline 运行目录。Qwen3.5 需要单独的 `requirements/qwen35.lock.txt`，不能把 7B 的 4.57.6 环境直接换模型。
