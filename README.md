...
最终方案采用 Qwen3.5-4B 后，我进一步在相同的 2×T4 环境下测试 vLLM 推理配置，观察并行方式和 batch size 对吞吐、延迟和显存的影响。

| 配置 | 推理吞吐 | 延迟 | 模型加载 / 682 条总耗时 | 结束显存 / GPU |
|---|---:|---:|---:|---:|
| TP2 | 1.167 req/s | 均值 856.8 ms；P95 1010.2 ms | 82.8 s / 675.0 s | 3.66 GiB |
| TP2-B32 | 1.430 req/s | 摊销 699.2 ms / 请求 | 83.2 s / **567.8 s** | 1.29 GiB |
| DP2-B16 | **1.549 req/s** | **摊销 645.6 ms / 请求** | 182.5 s / 630.4 s | 3.14 / 3.03 GiB |

DP2-B16 的推理阶段吞吐比 TP2-B32 高 **8.3%**，但模型加载更慢，因此单次 682 条任务的端到端时间更长。完整配置、测试口径、显存快照和预测一致率见 [Qwen3.5-4B vLLM 双卡推理实验](docs/qwen35_vllm_serving.md)。

DP2-B16 来自另一工作区的 Kaggle 实测；对应实现尚未同步到当前 Git 版本。表中保留的是实测结果，不表示当前 checkout 可以复现该配置。
