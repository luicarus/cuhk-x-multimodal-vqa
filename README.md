...
模型权重不提交到 Git。Notebook 可以下载固定 revision，或读取带来源收据的私有 Kaggle Input。ZIP 产物同样由本地私有存储或 Kaggle Dataset 管理；运行前必须把本地打包命令输出的 `manifest_sha256` 填入 Notebook，避免执行来源不明但内部哈希自洽的包。
