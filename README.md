...
旧 DP2-B16 记录的吞吐为 1.549 req/s；新 B32 实测吞吐低 3.2%，但并行初始化使模型加载比旧串行版本少 85.4 秒，端到端时间少 70.6 秒。批次和启动方式同时变化，这不是单变量对照。表中显存为运行结束时每卡空闲量，不是峰值；新运行没有可用的 TTFT 分布。完整指标、测量口径及限制见 [Qwen3.5-4B vLLM 双卡推理实验](docs/qwen35_vllm_serving.md)。
