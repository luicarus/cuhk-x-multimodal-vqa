...
| 配置 | 请求批次 / `max_num_seqs` | 推理吞吐 | 请求延迟 P95 | 模型加载 / 682 条总耗时 |
|---|---:|---:|---:|---:|
| TP2 | 逐条提交 | 1.167 req/s | 1010 ms | 82.8 s / 675.0 s |
| TP2-B32 | 32 | 1.430 req/s | 未记录 | 83.2 s / **567.8 s** |
| DP2-B16 | Runner 每批 16；DP=2、TP=1 | **1.549 req/s** | 未记录 | 182.5 s / 630.4 s |

在这组 682 请求上，DP2-B16 的推理阶段吞吐比 TP2-B32 高 8.3%，但启动时间更长，端到端耗时反而多 62.6 秒。这里将 DP 的 16 表述为 Runner 的请求批次上限；现有运行记录没有验证每个 replica 的上限或全局并发，因此不把它换算成 global concurrency。

DP2-B16 是另一工作区生成的 Kaggle 实测，实现尚未同步进当前 Git 版本；它是实验记录，不是当前 checkout 可复现的配置。完整口径、摊销批次时间、显存快照和限制见 [Qwen3.5-4B vLLM 双卡推理实验](docs/qwen35_vllm_serving.md)。
