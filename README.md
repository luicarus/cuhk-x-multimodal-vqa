...
| 已测配置 | Runner 总批次 / 每 replica `max_num_seqs` | 推理吞吐 | 模型加载 / 682 条端到端耗时 | 运行结束空闲显存 / GPU |
|---|---:|---:|---:|---:|
| TP2 | 1 / 1 | 1.167 req/s | 82.8 s / 675.0 s | 3.66 GiB |
| TP2-B32 | 32 / 32 | 1.430 req/s | 83.2 s / **567.8 s** | 1.29 GiB |
| **DP2-B32（最新）** | 32 / 16 | **1.500 req/s** | **97.1 s / 559.8 s** | 2.22 GiB |

最新 DP2-B32 完成 682 条请求，22 个批次（21×32、末批 10），平均摊销 666.6 ms/请求。相较 TP2-B32，推理吞吐高 4.9%，端到端时间短 8.1 秒；模型加载则多 13.9 秒。两个 DP 副本的加载时间分别为 88.9 / 85.3 秒，整体加载用时 97.1 秒，表明两份模型初始化已重叠。

旧 DP2-B16 记录的吞吐为 1.549 req/s；新 B32 实测吞吐低 3.2%，但并行初始化使模型加载比旧串行版本少 85.4 秒，端到端时间少 70.6 秒。批次和启动方式同时变化，这不是单变量对照。表中显存仍是这次已完成运行的结束空闲量，不是峰值；这次结果来自旧监控版本，没有 TTFT 分布。新 Notebook 会在重跑时把逐请求 TTFT 分布写入 `run_summary.json`，并以 NVML 每 100 ms 采样设备级显存峰值。完整指标、测量口径及限制见 [Qwen3.5-4B vLLM 双卡推理实验](docs/qwen35_vllm_serving.md)。
